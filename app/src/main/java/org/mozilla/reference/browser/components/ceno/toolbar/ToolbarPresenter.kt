...
package org.mozilla.reference.browser.components.ceno.toolbar
...
import org.mozilla.reference.browser.components.ceno.toolbar.internal.URLRenderer

/**
 * CENO: fork of mozilla's ToolbarFeature a-c, allows specific urls to be hidden in address bar
...
class ToolbarPresenter(
    ...
    private val hiddenAddressList: List<String>? = null
) {
    @VisibleForTesting
    internal var renderer = URLRenderer(toolbar, urlRenderConfiguration)

    val logger = Logger("ToolbarPresenter")
    ...
    internal fun render(state: BrowserState) {
        ...
        if (tab != null) {
            if(hiddenAddressList != null
                && hiddenAddressList.contains(tab.content.url))
            {
                renderer.post("")
            }
            else {
